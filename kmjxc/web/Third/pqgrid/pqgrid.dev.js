...
(function($) {
    ...
    _pG._renderCell = function(objP) {
        ...
        if (dataIndx.indexOf('.') > -1) {
            ...
            if (Indxs != null && Indxs != 'undefined' && Indxs.length > 0) {
                var obj;
        ...
        if (column.render) {
            ...
            dataCell = cellData;// rowData[dataIndx];
        }

        if (dataType === "timestamp" && dataCell != "" && dataCell != null && dataCell != 'undefined' && dataCell > 0) {
            
            dataCell = distanceOfTimeInWords(dataCell);
        } else if (dataType == "bool") {
            if (dataCell == "false") {
                dataCell = "否";
            } else if (dataCell == "true") {
                dataCell = "是";
            }
        } else if (dataType === "date" && dataCell != "" && dataCell != null && dataCell != 'undefined' && dataCell > 0) {
            dataCell = distanceOfTimeInWords(dataCell);
        }
    ...
    _p.add = function(objP) {
        ...
        if (isSelected == null) {
            ...
            var ret = this._boundRow(objP),
                    $tr = ret;
            selectedRows.push({ rowIndx: rowIndx, rowIndxPage: rowIndxPage });
    ...
    fn.UpdateData = function (dataIndx, rowIndexPage,newData) {
        var that = this;
        if (dataIndx.indexOf('.') > -1) {
            var indxs = dataIndx.split('.');
            if (indxs.length > 0) {
                var len = indxs.length;
                switch (len) {
                    case 2:
                        this.data[rowIndexPage][indxs[0]][indxs[1]] = newData;
                        break;
                    case 3:
                        this.data[rowIndexPage][indxs[0]][indxs[1]][indxs[2]] = newData;
                        break;
                    case 4:
                        this.data[rowIndexPage][indxs[0]][indxs[1]][indxs[2]][indxs[3]] = newData;
                        break;

                }
            }
        } else {
            this.data[rowIndexPage][dataIndx] = newData;
        }
    }
    fn.saveEditCell = function() {
        ...
        var $td = this.$td_edit,
                ...
                prevVal = this.getcellData(dataIndx, this.data[rowIndxPage]);
        ...
        if (rowIndxPage != null) {
            ...
            if (dataCell != prevVal) {
                ...
                this.UpdateData(dataIndx, rowIndxPage, dataCell);
                obj.data = this.data;
    ...
    fn._editCell = function($td) {
        ...
        if (column.editor) {
            ...
            $cell.children().html(that.getcellData(dataIndx,that.data[rowIndxPage]));
        }
        var that = this;
        window.setTimeout(function() {
            if (that.$td_edit != null) {
    ...
    fn.getcellData = function (dataIndx,rowData) {
        var cellData = "";
        var obj;
        if (dataIndx.indexOf('.') > -1) {
            var indxs = dataIndx.split('.');
    ...
    fn._sortLocalData = function(dataIndx, dir, dataType, data) {
        ...
        function innerSort() {
            ...
            function sort_string(obj1, obj2) {
                ...
                iter++;
                var val1 = that.getcellData(dataIndx, obj1);//  obj1[dataIndx];                
                var val2 = that.getcellData(dataIndx, obj2); //obj2[dataIndx];
                val1 = val1 ? val1 : "";
                val2 = val2 ? val2 : "";
                if (val1 > val2) {
                    return 1;
